...
*This lab examines closeness centrality and centralization using the `closeness()` and `centralization()` functions in the `sna` package.*
...
Before we calculate the closeness scores, recall that in a graph with multiple components (i.e. disconnected subgraphs) individuals who cannot be reached will have a closeness score of zero. So, what do we do? Call it a day and go home?

Actually, what often happens in practice is that researchers will take the "first component" of the graph and analyze the closeness scores for that graph. The first component is the largest subgraph in a graph. We can see this visually in the following plot:
...
set.seed ( 507 )
gplot( gaNetU, gmode = "graph", vertex.col = comp.col, main = "Get Along With Network (Undirected)", sub = "first component in green" )

```

To get the closeness centrality scores, we can use a function called `get.inducedSubgraph()` in the `sna` package to extract a subgraph that meets a condition. In this case, we want to take those cases who are in the first component. Let's walk through the pieces.
...
closeness( gaNetUcomp, gmode="graph" ) / n.nodes 
...
set.seed ( 507 )
gplot( piNetD, gmode = "digraph", vertex.col = comp.col, main = "Power Network (Directed)", sub = "first component in red" )

```

Now, let's again use the `get.inducedSubgraph()` function in the `sna` package to extract a subgraph that meets the first component condition.
...
#unstandardized closeness.
closeness( piNetDcomp, gmode="graph" ) / n.nodes 
